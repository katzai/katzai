...
pydroid_path = "/data/user/0/ru.iiec.pydroid3/files/aarch64-linux-android/lib/python3.13/site-packages"
if os.path.exists(pydroid_path) and pydroid_path not in sys.path:
    sys.path.append(pydroid_path)
    os.environ["PYTHONPATH"] = pydroid_path
...
# Replace the placeholder below with your actual SendGrid API key

SENDGRID_API_KEY = "PASTE_YOUR_SENDGRID_API_KEY_HERE"  # ← PUT YOUR API KEY HERE
SENDGRID_FROM_EMAIL = "katzai878@gmail.com"  # ← Your verified sender email

# Legacy Gmail config (not used with SendGrid, but kept for reference)
...
def send_otp_email(to_email, otp_code, purpose='verification'):
    ...
    try:
        ...
        if SENDGRID_API_KEY == "PASTE_YOUR_SENDGRID_API_KEY_HERE":
            logger.error("SendGrid API key not configured!")
            return False
        
        # Prepare email content
        if purpose == 'reset':
            subject = 'Reset Your Katz AI Pro Password'
            title = 'Password Reset Code'
        else:
            subject = 'Verify Your Katz AI Pro Account'
...
<p style="color: #666; text-align: center;">Code expires in {OTP_EXPIRY_MINUTES} minutes.</p>
        ...
        logger.info(f'✅ OTP sent via SendGrid to {to_email} for {purpose}')
        return True
